...
# 6.1 住院期间平均值计算相关性 #--------------------------------------------------------------
# 住院天数大于2小于40的患者 #---------------------------------------------------
folder_name <- "D:\\Lab_project\\2026work\\Markfu\\PLOT\\sepsis\\correlation\\住院天数大于2小于40的患者脓毒症相关指标相关性\\住院期间平均值"
if (!dir.exists(folder_name)) {dir.create(folder_name)print("文件夹创建成功")} else {
  print("文件夹已存在")}
...
df_lab1 <- df_lab %>% select(hadm_id,TIME,valuenum,label,Outcome_Mortality,Length_of_Stay,In_hospital_Mortality_Time,shock)
