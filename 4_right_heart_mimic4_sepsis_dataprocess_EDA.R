...
df_lab_log <- readRDS("mimic4_Sepsis脓毒症相关指标log.rds") 
colnames(df_lab_log)
...
# 筛选出首诊断sepsis的患者 #--------------------------------------------------
{
  ...
  # 保留第一次出现的列，删除重复的
  df_diagnose <- df_diagnose[, !duplicated(colnames(df_diagnose))]
  
  # 验证
  colnames(df_diagnose)
  
  # 然后重新运行 filter
  df_diagnose1 <- df_diagnose %>% filter(hadm_id %in% df_lab_log$hadm_id)
  df_diagnose1 <- df_diagnose %>% filter(seq_num == 1) %>% filter(grepl("sepsis|septic", long_title, ignore.case = TRUE))
  length(unique(df_diagnose1$hadm_id))
  # [1] 10711
  df_diagnose_sepsis <- df_diagnose %>% filter(hadm_id %in% df_diagnose1$hadm_id)
  
  
}

setwd("D:\\Lab_project\\2026work\\sepsis\\DATA\\sepsis\\derived_data\\sepsis_seqnum1")
df_input <- readRDS("sepsis_seqnum1_床边用药信息.rds")
df_chartevents <- readRDS("sepsis_seqnum1_床边监护.rds")
length(unique(df_input$hadm_id))
# [1] 6815
length(unique(df_chartevents$hadm_id))
# [1] 6825

df_input <- df_input %>% select(-c(itemid...26)) %>% rename(itemid = itemid...7)
...
df_admit <- df_lab_log %>% distinct(hadm_id,admittime,edregtime)
df_input1 <- df_input %>%
...
df_chartevents1 <- df_chartevents %>% filter(hadm_id %in% common_ids) %>%
  mutate(across(c(charttime, storetime), ~ as.POSIXct(as.numeric(.)*86400, origin = "1899-12-30", tz = "UTC"))) %>%
  mutate(itemid = as.numeric(itemid),hadm_id = as.character(hadm_id)) %>%  # 转为数值
  left_join(df_item, by = "itemid") %>%
  left_join(df_admit %>% mutate(hadm_id = as.character(hadm_id)), by = "hadm_id")

df_lab_sepsis <- df_lab_log %>% filter(hadm_id %in% df_diagnose_sepsis$hadm_id) %>%
  filter(hadm_id %in% common_ids)

{
  ...
  # 直接计算有ABP的患者数
  abp_patients <- df_chartevents1 %>%
    filter(label %in% abp_labels) %>%
    pull(hadm_id) %>%
    unique()
  total_patients <- df_chartevents1 %>%
    pull(hadm_id) %>%
    unique()
...
# 总患者数: 6815至少有1种有创动脉血压的患者数: 2350覆盖率: 34.48 %无任何有创动脉血压的患者数: 4465

df_diagnose_shock <- df_diagnose_sepsis %>%
  filter(grepl("shock", long_title, ignore.case = TRUE)) %>%
  filter(long_title != "Severe sepsis without septic shock")

length(unique(df_diagnose_shock$hadm_id))
unique(df_diagnose_shock$long_title)
# [1] "Septic shock"                                        "Severe sepsis with septic shock"                    
# [3] "Other shock"                                         "Shock, unspecified"                                 
# [5] "Postprocedural septic shock, initial encounter"      "Cardiogenic shock"                                  
# [7] "Other shock without mention of trauma"               "Hypovolemic shock"                                  
# [9] "Toxic shock syndrome"                                "Postoperative shock, cardiogenic"                   
# [11] "Other postprocedural shock, initial encounter"       "Postoperative shock, septic"                        
# [13] "Shock due to anesthesia, not elsewhere classified"   "Postoperative shock, other"                         
# [15] "Postprocedural cardiogenic shock, initial encounter" "Traumatic shock"                                    
# [17] "Shock due to anesthesia, initial encounter"          "Traumatic shock, initial encounter"                 
# [19] "Postoperative shock, not elsewhere classified" 

# 确定所有休克群体休克开始时间 #-------------------------------------------------------------
setwd("D:\\Lab_project\\2026work\\sepsis\\DATA\\sepsis\\original_data\\sepsis_seqnum1")
df_hospital <- read.xlsx("sepsis_seqnum1_icustays.xlsx") %>% filter(hadm_id %in% df_diagnose_shock$hadm_id) %>%
  mutate(across(c(intime, outtime), ~ as.POSIXct(as.numeric(.) * 86400, origin = "1899-12-30", tz = "UTC")))

df_admitall <- read.xlsx("mimic4_spesis_住院信息.xlsx")

df_filter_input <- df_input1 %>% filter(hadm_id %in% df_diagnose_shock$hadm_id)
df_filter_chartevent <- df_chartevents1 %>% filter(hadm_id %in% df_diagnose_shock$hadm_id)
df_filter_lab <- df_lab_sepsis %>% filter(hadm_id %in% df_diagnose_shock$hadm_id)
length(unique(df_lab_sepsis$hadm_id))
{
  # 0. 基础 ID 统一与清理
  clean_ids <- function(df) {
    df %>% mutate(across(any_of(c("subject_id", "hadm_id", "stay_id")), as.numeric))
  }
  sofa_bridge <- df_hospital %>%
    select(subject_id, hadm_id, stay_id, intime, outtime) %>%
    distinct() %>%
    clean_ids()
 
  # 补全 Lab 表的 stay_id
  df_lab_with_stay <- df_filter_lab %>%
    clean_ids()
  # 1. 提取所有候选点：低血压、升压药、高乳酸
  # 候选 1：所有的低血压事件 (MAP <= 65)
  df_hypo_candidates <- df_filter_chartevent %>%
    filter(itemid %in% c(220052, 220181) & valuenum > 0 & valuenum <= 65) %>%
    select(hadm_id,map_time = charttime, map_value = valuenum) %>%
    clean_ids()
  # 候选 2：所有的血管升压药使用记录
  vasso_itemids <- c(221906, 221662, 222315, 221289, 221749)
  df_vaso_candidates <- df_filter_input %>%
    filter(itemid %in% vasso_itemids) %>%
    select(hadm_id,vaso_start_time = starttime) %>%
    clean_ids()
  # 候选 3：所有的高乳酸记录 (Lactate >= 2.0)
  df_lac_candidates <- df_lab_with_stay %>%
    filter(grepl("Lactate", label, ignore.case = TRUE) & valuenum >= 2) %>%
    select(hadm_id,lactate_time = charttime, lac_value = valuenum) %>%
    clean_ids()
  
  # 2. 以低血压为中心，进行双向时间窗验证
  df_valid_hypotension <- df_hypo_candidates %>%
    # 辅助验证 1：在低血压事件后 12 小时内，必须有血管升压药的使用
    inner_join(df_vaso_candidates, by = "hadm_id", relationship = "many-to-many") %>%
    filter(as.numeric(difftime(vaso_start_time, map_time, units = "hours")) >= 0 &
             as.numeric(difftime(vaso_start_time, map_time, units = "hours")) <= 12) %>%
    # 辅助验证 2：在低血压事件前后 12 小时内，必须有高乳酸记录
    inner_join(df_lac_candidates, by = "hadm_id", relationship = "many-to-many") %>%
    filter(abs(as.numeric(difftime(map_time, lactate_time, units = "hours"))) <= 12) %>%
    # 清理冗余：一个低血压时刻可能匹配到多条药或乳酸记录，我们只保留最近的作为记录
    group_by(hadm_id, map_time, map_value) %>%
    summarise(
      nearest_vaso_time = vaso_start_time[which.min(abs(difftime(map_time, vaso_start_time)))],
      nearest_lac_time = lactate_time[which.min(abs(difftime(map_time, lactate_time)))],
      .groups = "drop"
    )

  
  df_final_shock_cohort <- df_valid_hypotension %>%
    mutate( shock_onset_time = map_time) %>%
    group_by(hadm_id) %>%
    slice_min(shock_onset_time, n = 1, with_ties = FALSE) %>%
    ungroup()
  cat("==========================================\n")
  cat("      新逻辑：低血压锚定 + 药/乳酸辅助验证\n")
  cat("==========================================\n")
  cat("出现过低血压(MAP<=65)人数: ", length(unique(df_hypo_candidates$hadm_id)), "\n")
  cat("匹配用药及高乳酸最终人数:  ", nrow(df_final_shock_cohort), "\n")
  cat("------------------------------------------\n")
  cat("Time Zero (shock_onset_time) 设定为：经过验证的【最早低血压发生时间】\n")
  print(head(df_final_shock_cohort %>% select(hadm_id, shock_onset_time, nearest_vaso_time, nearest_lac_time)))
}
# 出现过低血压(MAP<=65)人数:  4220 
# 匹配用药及高乳酸最终人数:   2513 

df_lab_shock <- df_lab_sepsis %>%
  filter(hadm_id %in% df_final_shock_cohort$hadm_id) %>%
  left_join(
    df_final_shock_cohort %>%
      mutate(hadm_id = as.character(hadm_id)) %>%
      select(hadm_id, shock_onset_time),
    by = "hadm_id"
  )

# 处理诊断没有休克患者
setwd("D:\\Lab_project\\2026work\\sepsis\\DATA\\sepsis\\original_data\\sepsis_seqnum1")
df_hospital_noshock <- read.xlsx("sepsis_seqnum1_icustays.xlsx") %>% filter(!hadm_id %in% df_diagnose_shock$hadm_id) %>%
  mutate(across(c(intime, outtime),
                ~ as.POSIXct(as.numeric(.) * 86400, origin = "1899-12-30", tz = "UTC")))

df_lab_sepsis_noshock <- df_lab_sepsis %>% filter(!hadm_id %in% df_diagnose_shock$hadm_id) %>%
  filter(hadm_id %in% df_hospital_noshock$hadm_id) %>%
  mutate(shock_onset_time = NA)

all_data <- rbind(df_lab_shock,df_lab_sepsis_noshock)

length(unique(all_data$hadm_id))
# 115

# setwd("D:\\Lab_project\\2026work\\sepsis\\DATA\\sepsis\\derived_data\\sepsis_seqnum1")
# write.csv(all_data,"lab_sepsis_seqnum1_filtered.csv")
# saveRDS(all_data,"lab_sepsis_seqnum1_filtered.rds")

setwd("D:\\Lab_project\\2026work\\sepsis\\DATA\\sepsis\\original_data\\sepsis_seqnum1")
df_hospital_all <- read.xlsx("sepsis_seqnum1_icustays.xlsx") %>%
  mutate(across(c(intime, outtime), ~ as.POSIXct(as.numeric(.) * 86400, origin = "1899-12-30", tz = "UTC")))
# 建立一个干净的 ID 映射表
id_bridge <- df_hospital_all %>%
  select(subject_id, hadm_id, stay_id, intime, outtime) %>%
  distinct() %>%
...
all_data_outcome  <- all_data %>% distinct(hadm_id,hosp_start_time,shock_onset_time, deathtime,hospital_expire_flag)
...
nrow(all_data_outcome1)
nrow(all_data_outcome)
# 判断icu结局
...
# 1. 提取首诊 ICU 期间的 labevents 记录
{
  ...
  df_first_icu_course <- all_data %>%
    ...
    filter(charttime >= (first_intime) & charttime <= first_outtime)
  cat("--- 提取完成 ---\n")
  cat("提取前化验记录总数: ", nrow(all_data), "\n")
...
# 筛选有心血管疾病患者 #--------------------------------------------------------
{
  ...
  heart_diseases_all_v2 <- df_diagnose_sepsis %>%
    mutate(
  ...
  # 筛选出有心血管疾病诊断的患者 hadm_id（唯一值）
  cvd_patient_hadm_ids <- heart_diseases_all_v2 %>%
    pull(hadm_id) %>%
    unique()
  # 可选：查看结果
  cat("=== 有心血管疾病诊断的患者统计 ===\n")
  cat("符合诊断的患者住院数（hadm_id）:", length(cvd_patient_hadm_ids), "\n")
  # 可选：生成汇总统计（按疾病类型分组）
  cvd_by_type <- heart_diseases_all_v2 %>%
    group_by(heart_type) %>%
    summarise(
      patient_count = n_distinct(hadm_id),
      record_count = n()
    ) %>%
  ...
  print(cvd_by_type)
  # === 有心血管疾病诊断的患者统计 ===
  #   符合诊断的患者住院数（hadm_id）: 3102 
  # 
  # === 各类型心血管疾病患者数 ===
  #   # A tibble: 1 × 3
  #   heart_type patient_count record_count
  # <chr>              <int>        <int>
  #   1 心衰                3102         4393
}
length(unique(df_first_icu_course$hadm_id[df_first_icu_course$hadm_id %in% cvd_patient_hadm_ids]))
df_first_icu_card_lab <- df_first_icu_course %>% filter(hadm_id %in% cvd_patient_hadm_ids)
unique(heart_diseases_all_v2$long_title)
# 筛选出有心损指标测量次数超过3的患者 #------------------------------------------
{
  library(dplyr)
  # ,"Creatine Kinase, MB Isoenzyme"
  # 1. 定义你需要的目标指标名称 (请务必确保这里的名称与 df_lab$label 中的完全一致)
  target_all <- c("Troponin T",
                  "Creatine Kinase, MB Isoenzyme"
  )
  # "CK-MB Index"
  # "Creatine Kinase, MB Isoenzyme"
  ...
  eligible_ids <- df_first_icu_card_lab %>%
    # filter(TIME <= 7 ) %>%
    # 只保留这三项指标的数据，且数值不为空
    ...
    pull(hadm_id)
  # 3. 使用这些 ID 过滤原始数据集
  df_final <- df_first_icu_card_lab%>%
...
{
df_first_icu_card_lab1 <- df_first_icu_card_lab %>%
  mutate(
...
unique(df_first_icu_card_lab1$this_icu_outcome)
# 检查是否存在负值（除了 charttime 允许有少量 buffer 负值外，其余通常应 >= 0）
cat("相对时间计算完成。摘要统计：\n")
summary(df_first_icu_card_lab1 %>% select(charttime_rel_icu, shock_onset_rel_icu, endpoint_rel_icu))
# charttime_rel_icu   shock_onset_rel_icu endpoint_rel_icu  
# Min.   : 0.000116   Min.   : 0.000      Min.   : 0.09444  
# 1st Qu.: 0.811806   1st Qu.: 0.027      1st Qu.: 4.34146  
# Median : 2.630556   Median : 0.154      Median : 9.22613  
# Mean   : 5.139337   Mean   : 1.112      Mean   :12.24081  
# 3rd Qu.: 6.679861   3rd Qu.: 0.594      3rd Qu.:15.55208  
# Max.   :74.495139   Max.   :18.200      Max.   :75.03958  
#                     NA's   :81770    
}

# 1. 提取首诊 ICU 期间的 Chartevents 记录
{
  df_first_icu_chartevent_course <- df_chartevents1 %>%
    filter(hadm_id %in% df_first_icu_card_lab1$hadm_id) %>%
  ...
  df_first_icu_chartevent_course1 <- df_first_icu_chartevent_course %>%
    mutate(
      # # 确保涉及计算的列均为时间格式
  ...
  # 打印摘要检查
  summary(df_first_icu_chartevent_course1 %>% select(charttime_rel_icu, shock_onset_rel_icu, endpoint_rel_icu))
}
# --- Chartevents 提取完成 ---
#   首诊患者人数:  1414 
# 提取出的监护记录总数:  3703523 
# charttime_rel_icu shock_onset_rel_icu endpoint_rel_icu  
# Min.   : 0.000    Min.   : 0.00       Min.   : 0.09444  
# 1st Qu.: 1.301    1st Qu.: 0.03       1st Qu.: 4.99023  
# Median : 3.601    Median : 0.15       Median :10.00835  
# Mean   : 6.200    Mean   : 1.10       Mean   :13.16841  
# 3rd Qu.: 8.106    3rd Qu.: 0.63       3rd Qu.:16.77025  
# Max.   :74.994    Max.   :18.20       Max.   :75.03958  
#                   NA's   :1217675   

# 统计每个 label 的测量人数
colnames(df_first_icu_card_lab1)
df_first_icu_card_lab2 <- df_first_icu_card_lab1 %>%  mutate(
  shock = case_when(is_shock_onset_in_this_icu == "TRUE" ~ 1,
                    TRUE ~ 0),
...
label_counts <- df_first_icu_card_lab2 %>%
  group_by(label) %>%
  summarise(
    ...
    total_n = n_distinct(df_first_icu_card_lab2$hadm_id),
    percent = n_patients/total_n
  ) %>%
  arrange(desc(n_patients))  # 按患者数降序排列
output_dir <- "D:\\Lab_project\\2026work\\sepsis\\PLOT\\sepsis\\EDA_table\\sepsis_seqnum1"
file_name <- "Table3.ICU实验室检查人数统计.xlsx"
write.xlsx(label_counts, file.path(output_dir, file_name))

# 实验室指标数据统计 #-----------------------------------------------------------------
# 统计每个 label 的测量人数
# 计算中位数和95%分位数
summary_stats <- df_first_icu_card_lab2 %>%
  summarise(
    中位数 = median(outtime_rel_icu, na.rm = TRUE),
    P95 = quantile(outtime_rel_icu, probs = 0.95, na.rm = TRUE),
    平均值 = mean(outtime_rel_icu, na.rm = TRUE),
    标准差 = sd(outtime_rel_icu, na.rm = TRUE),
    最小值 = min(outtime_rel_icu, na.rm = TRUE),
    最大值 = max(outtime_rel_icu, na.rm = TRUE)
  )
print(summary_stats)
# 基础直方图 + 密度曲线
# saveRDS(df_first_icu_card_lab2,"sepsis伴有心衰患者实验室检查数据.rds")

# saveRDS(df_first_icu_chartevent_course1,"sepsis伴有心衰患者chartevents.rds")
...
summary_stats <- df_first_icu_card_lab2 %>%
  summarise(
    中位数 = median(outtime_rel_icu, na.rm = TRUE),
    P75 = quantile(outtime_rel_icu, 0.75, na.rm = TRUE),
    P95 = quantile(outtime_rel_icu, 0.95, na.rm = TRUE),
    平均值 = mean(outtime_rel_icu, na.rm = TRUE)
  )
...
# 然后再绘图
density_obj <- density(df_first_icu_card_lab2$outtime_rel_icu, na.rm = TRUE)
y_max <- max(density_obj$y)

library(ggplot2)

ggplot(df_first_icu_card_lab3, aes(x = outtime_rel_icu)) +
  ...
  # 添加中位数线
  geom_vline(xintercept = summary_stats$中位数, 
             color = "red", 
             linetype = "dashed", 
             size = 1) +
  # 添加95%分位数线
  geom_vline(xintercept = summary_stats$P95, 
             color = "blue", 
             linetype = "dashed", 
             size = 1) +
  # 添加均值线
  geom_vline(xintercept = summary_stats$P75, 
             color = "green", 
             linetype = "dotted", 
             size = 1) +
  # 添加文本标注
  annotate("text", 
           x = summary_stats$中位数, 
           y = max(density(df_first_icu_card_lab2$outtime_rel_icu, na.rm = TRUE)$y) * 0.9,
           label = paste("中位数 =", round(summary_stats$中位数, 2)),
           color = "red", 
           hjust = -0.1) +
  annotate("text", 
           x = summary_stats$P95, 
           y = max(density(df_first_icu_card_lab2$outtime_rel_icu, na.rm = TRUE)$y) * 0.8,
           label = paste("P95 =", round(summary_stats$P95, 2)),
           color = "blue", 
           hjust = -0.1) +
  labs(title = "outtime_rel_icu 分布直方图与密度曲线",
       subtitle = paste("样本量 =", nrow(df_first_icu_card_lab2), 
                        ", 缺失值 =", sum(is.na(df_first_icu_card_lab2$outtime_rel_icu))),
       x = "outtime_rel_icu (小时/天)", 
       y = "密度") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14),
        plot.subtitle = element_text(hjust = 0.5, size = 10))

df_first_icu_card_lab3 <- df_first_icu_card_lab2 %>% filter(outtime_rel_icu <= 15) %>% filter(outtime_rel_icu >= 2) 
# %>% filter(hadm_id %in% eligible_ids)
length(unique(df_first_icu_card_lab3$hadm_id))
# [1] 1331

label_counts <- df_first_icu_card_lab3 %>%
  group_by(label) %>%
  summarise(
    n_measurements = n(),  # 总测量次数
    n_patients = n_distinct(hadm_id),  # 不同患者数量（添加逗号）
    measurements_per_patient = n_measurements / n_patients,  # 平均每人测量次数
    total_n = n_distinct(df_first_icu_card_lab3$hadm_id),
    percent = n_patients/total_n
  ) %>%
  arrange(desc(n_patients))  # 按患者数降序排列
output_dir <- "D:\\Lab_project\\2026work\\sepsis\\PLOT\\sepsis\\EDA_table\\sepsis_seqnum1"
file_name <- "Table4.ICU实验室检查人数统计.xlsx"
write.xlsx(label_counts, file.path(output_dir, file_name))

length(unique(df_first_icu_card_lab3$hadm_id[df_first_icu_card_lab3$shock == 1]))

unique(df_first_icu_card_lab3$label)
...
# 筛选出有心损指标测量次数超过3的患者 #------------------------------------------
{
  ...
  # 2. 执行筛选逻辑
  # 我们先通过分组计算，找出符合条件的 stay_id（或 subject_id）
  eligible_ids <- df_first_icu_card_lab3 %>%
    # filter(TIME <= 7 ) %>%
    # 只保留这三项指标的数据，且数值不为空
    filter(label %in% target_all, !is.na(valuenum)) %>%
    # 按患者和指标分组，统计每人每项指标测了多少次
...
# [1] "符合条件的患者数量: 672"
{
  ...
  # 1. 定义你需要的目标指标名称
  target_all <- c("Creatine Kinase, MB Isoenzyme", "Troponin T")
  
  # 2. 执行筛选逻辑（"或"的关系，只要有一项即可）
  eligible_ids1 <- df_first_icu_card_lab3 %>%
    # 只保留这两项指标的数据，且数值不为空
    filter(label %in% target_all, !is.na(valuenum)) %>%
    # 按患者分组，统计每人测量了其中几项（只要至少有一项就算符合）
    group_by(hadm_id) %>%
    summarise(measured_labels = n_distinct(label)) %>%
    # 筛选出至少测量了其中一项的患者（即 measured_labels >= 1）
    filter(measured_labels >= 1) %>%
    pull(hadm_id)
  ...
  # 查看筛选结果
  print(paste("符合条件的患者数量:", length(eligible_ids1)))
}
{
  library(dplyr)
  
  # 假设数据框名为 df，包含: hadm_id, label, charttime, valuenum 等字段
  
  # Step 1: 按 hadm_id 汇总——统计不同 label 数量和采样点总数
  hadm_summary <- df_first_icu_card_lab2 %>%
    group_by(hadm_id) %>%
    summarise(
      n_labels  = n_distinct(label),       # 不同生物标志物的种类数
      n_samples = n(),                     # 总采样点数（所有 label 的记录数）
      n_timepoints = n_distinct(charttime) # 不同时间点的数量（可选）
    ) %>%
    ungroup()
  
  # Step 2: 设定阈值进行筛选
  # 阈值可以用分位数，避免硬编码
  label_thr  <- quantile(hadm_summary$n_labels,  0.72, na.rm = TRUE)
  sample_thr <- quantile(hadm_summary$n_samples, 0.71, na.rm = TRUE)
  
  selected_hadm <- hadm_summary %>%
    filter(n_labels >= label_thr,
           n_samples >= sample_thr) %>%
    arrange(desc(n_labels), desc(n_samples))
  
  # Step 3: 用筛出的 hadm_id 提取原始数据
  df_selected <- df_first_icu_card_lab2 %>%
    filter(hadm_id %in% selected_hadm$hadm_id) %>% 
    filter(!hadm_id %in% eligible_ids)
  
  length(unique(df_selected$hadm_id))
  
  # 查看结果
  head(df_selected$hadm_id)
  nrow(df_selected$hadm_id)   # 共筛出多少个 hadm_id
}

# 筛选目标患者
{
  first_icu_lab <- df_first_icu_card_lab3 %>% filter(hadm_id %in% eligible_ids1)
